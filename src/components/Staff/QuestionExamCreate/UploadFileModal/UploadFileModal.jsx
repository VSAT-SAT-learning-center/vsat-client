...
function UploadFileModal({ setIsShowUploadFileModal }) {
  ...
  const handleDragOver = (e) => {
    e.preventDefault();
    setDragging(true); // Highlight the drop area when dragging
  };

  // Handle drag-leave event
  const handleDragLeave = () => {
    setDragging(false); // Remove highlight when the user leaves the drop area
  };
  ...
  const readExcelFile = (file) => {
    ...
    reader.onload = (e) => {
      ...
      // Chuyển đổi dữ liệu từ file Excel sang JSON với cấu trúc mong muốn
  ...
  const formatExcelData = (rows) => {
    ...
    rows.forEach((row) => {
      // Kiểm tra nếu câu hỏi chưa tồn tại
      const existingQuestion = result.find(
        (q) => q.content === row.Content
      );

      // Nếu chưa tồn tại, tạo mới câu hỏi
      if (!existingQuestion) {
        result.push({
          level: row.Level,
          skill: row.Skill,
        ...
        // Nếu câu hỏi đã tồn tại, thêm câu trả lời vào mảng answers
