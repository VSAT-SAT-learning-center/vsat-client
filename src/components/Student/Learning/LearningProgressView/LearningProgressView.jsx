...
function LearningProgressView({
  ...
  useEffect(() => {
    const fetchProgressData = async () => {
      try {
        setIsWaiting(true);
        const responose = await apiClient.get(
          `/target-learnings/${target?.id}/recent-learning`
        );
  ...
  const handleMarkAsCompleted = () => {
    ...
    if (totalIncompleteUnits > 0) {
      ...
      setShowMarkPopupData(
        `There are ${totalIncompleteUnits} units not complete of out ${totalUnits}. Do you want continue mark this completed?`
      );
    } else {
      setShowMarkPopup(true);
      setShowMarkPopupData(
        "All units across all sections are complete. Do you want to mark this completed?"
      );
    }
  };
  return (
    <>
      {showMarkPopup && (
        <MarkCompletePopup
          target={target}
          message={markPopupData}
          setShowMarkPopup={setShowMarkPopup}
        ...
        <div className={cx("learning-progress-view-container")}>
          <div className={cx("learning-progress-view-header")}>
            ...
            {user.role === "Teacher" && target?.status !== "Completed" ? (
              <button
                className={cx("progress-mark-btn")}
          ...
          {isWaiting ? (
            ...
            <div className={cx("learning-progress-view-content")}>
              {/* <div className={cx("learning-progress-sidebar-no-container")}></div> */}
              <div
                className={cx(
                  ...
                  progressData?.map((progress) => (
                    <div
                      className={cx("learning-progress-introduction", {
                        "introduction-active":
                          activeProgress.targetLearningDetailId ===
                          progress.targetLearningDetailId,
                      })}
                    ...
                    >
                      ...
                      <div className={cx("introduction-infor")}>
                        <div className={cx("infor-title")}>
                          SAT {progress.section.name}
                        </div>
                        <div className={cx("infor-details")}>
                          <span className={cx("infor-units")}>
                            {progress.totalUnitAreaCount} TOPICS
                          </span>{" "}
                          -{" "}
                          <span className={cx("infor-skills")}>
                            {progress.totalLessonCount} LESSONS
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              <div
                className={cx(
                  activeProgress?.unitProgresses.length > 0
              ...
              >
                {activeProgress?.unitProgresses.length > 0 ? (
                  activeProgress?.unitProgresses.map((unitProgress) => (
                    <div
                      className={cx("learning-progress-item")}
                      key={unitProgress?.unitProgressId}
                    ...
                    >
                      <div className={cx("learning-material-title")}>
                        ...
                        <div className={cx("title-text")}>
                          {unitProgress?.unit.title}
                        </div>
                      </div>
                      <div className={cx("learning-material-about")}>
                        {unitProgress?.unit.description}
                      </div>
                      <div className={cx("learning-material-main")}>
                        <div className={cx("learning-material-config")}>
                          <div className={cx("infor-level")}>
                            <i
                              className={cx(
                                "fa-sharp fa-light fa-layer-group",
                                "level-icon"
                              )}
                            ></i>
                            <span className={cx("level-text")}>
                              {unitProgress?.unit?.level.name}
                            </span>
                          </div>
                          <div className={cx("infor-detail")}>
                            <div className={cx("detail-item")}>
                              ...
                              <span className={cx("detail-text")}>
                                {unitProgress.unit.unitAreaCount} Topics
                              </span>
                            </div>
                            <div className={cx("detail-item")}>
                              <i
                              ...
                              ></i>
                              <span className={cx("detail-text")}>
                                {unitProgress.unit.lessonCount} Lessons
                              </span>
                            </div>
                          </div>
                        </div>
                        <div
                          className={cx("learning-material-progress", {
                            "normal-progress":
